...
const consultingServices: Service[] = [
  ...
  { name: "산업보건지도사 컨설팅", description: "산업보건지도사의 전문적인 지식과 경험을 바탕으로 사업장 보건 문제에 대한 심층 진단과 종합적인 솔루션을 제공합니다.", imageSrc: "/1.jpg" },
];
                ...
                <motion.div key={currentIndex} variants={variants} initial="enter" animate="center" exit="exit" className="absolute top-0 left-0 w-full">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center w-full">
                    <div className="text-center md:text-left">
                      <motion.h1 variants={titleVariants} className="text-4xl lg:text-5xl font-extrabold leading-tight mb-4 whitespace-pre-line">{currentSlide.title}</motion.h1>
                      <motion.p variants={subtitleVariants} className="text-lg lg:text-xl text-blue-100 mb-8 whitespace-pre-line">{currentSlide.subtitle}</motion.p>
                    </div>
            ...
            {featureData.map((feature) => (
              <div key={feature.title} className={`${feature.bgColor} rounded-3xl p-8 flex flex-col justify-between aspect-square shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300`}>
                <div className="flex justify-between items-start">
                  <Image src={feature.iconSrc} alt={`${feature.title} icon`} width={48} height={48} unoptimized />
